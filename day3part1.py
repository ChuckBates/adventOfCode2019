...
def function( wireOne, wireTwo ):
    ...
    for movement in wireOnePathMovements:
        ...
        if direction == "R":
            while magnitude > 0:
                wireOneX += 1
                wireOnePath.append( (wireOneX,wireOneY) )
                magnitude -= 1                    
        if direction == "L":
            while magnitude > 0:
                wireOneX -= 1
    ...
    for movement in wireTwoPathMovements:
        ...
        if direction == "R":
            while magnitude > 0:
                wireTwoX += 1
                wireTwoPath.append( (wireTwoX,wireTwoY) )
                magnitude -= 1                    
        if direction == "L":
            while magnitude > 0:
                wireTwoX -= 1
    ...
    crossedPaths = []
    temp = set(wireOnePath)
    crossedPaths = [value for value in wireTwoPath if value in temp]
    distance = 0
    for crossedPath in crossedPaths:
        if distance == 0 or distance > (abs(crossedPath[0]) + abs(crossedPath[1])):
            distance = (abs(crossedPath[0]) + abs(crossedPath[1]))
    return distance
