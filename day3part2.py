...
def function( wireOne, wireTwo ):
    wireOnePathMovements = wireOne.split(",")
    wireTwoPathMovements = wireTwo.split(",")
    wireOneX = 0
    wireOneY = 0
    wireOnePath = {}
    wireOneCounter = 0
    for movement in wireOnePathMovements:
        ...
        if direction == "R":
            while magnitude > 0:
                wireOneX += 1
                wireOneCounter += 1
                if (wireOneX,wireOneY) not in wireOnePath:
                    wireOnePath[(wireOneX,wireOneY)] = wireOneCounter
                magnitude -= 1                    
        if direction == "L":
            while magnitude > 0:
                wireOneX -= 1
    ...
    wireTwoPath = {}
    wireTwoCounter = 0
    for movement in wireTwoPathMovements:
        direction = movement[:1]
        magnitude = int(movement[1:])
        if direction == "R":
            while magnitude > 0:
                wireTwoX += 1
                wireTwoCounter += 1
                if (wireTwoX,wireTwoY) not in wireTwoPath:
                    wireTwoPath[(wireTwoX,wireTwoY)] = wireTwoCounter
                magnitude -= 1                    
        if direction == "L":
            while magnitude > 0:
                wireTwoX -= 1
